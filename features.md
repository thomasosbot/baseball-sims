...
### v1.4 — Opening Day 2026 (completed)
...
### v1.5 — LLM enrichment + transparency (completed Apr 2026)

| Feature | Module(s) | Status |
|---------|-----------|--------|
| **LLM-generated pick analysis** | `src/betting/narrative.py`, `scripts/run_daily.py` | Done — Claude Opus 4.7 drafts 5-6 sentence pick brief in @Ozzy_Analytics snark voice. Fed structured brief from `statcast_summary.py` rollups (xwOBA, barrel%, K%, hard-hit% with vsL/vsR splits) + edge math. Falls back silently to rule-based copy on any failure. ~$0.15/day cost. |
| **LLM post-game recap** | `src/betting/narrative.py` | Done — `generate_pick_recap()` produces 2-3 sentence recap per pick from MLB play-by-play highlights (HRs, late scoring, decisions, venue, profit). |
| **LLM newsletter opener** | `src/newsletter/sender.py` | Done — `generate_day_story()` writes 4-6 sentence narrative opener summarizing day's picks + previous results. |
| **Statcast rollup pre-compute** | `src/features/statcast_summary.py` | Done — `save_rollup()` builds and pickles per-player rollups (1248 hitters, 723 pitchers) to `data/processed/statcast_rollup_2025.pkl` (~520 KB, committed). `get_rollups()` prefers the precomputed file so CI runs don't need raw multi-GB Statcast cache. Fix shipped after Apr 24 CI run silently fell back to rule-based copy due to missing rollup. |
| **Name resolver cache** | `src/features/name_resolver.py` | Done — moved disk cache from gitignored `data/cache/` to `data/processed/name_to_mlbam.json` so CI can use it. |
| **Expandable Analysis on pick rows** | `site/templates/index.html` | Done — homepage pick rows now have an Analysis toggle showing the LLM narrative inline. |
| **30-day check-in flyer** | `site/public/30-days.html` | Done — hand-built one-page dark-mode flyer (not regenerated by `site/generate.py`). Personal lede, $10K → $32K transformation banner with inline SVG chart, stat tiles, run-line / underdog breakdown, totals-honesty note, 9-tile method grid (Monte Carlo, Statcast+Bayesian, hard-hit & barrel, advanced K & whiff, sabermetrics, weather/park, speed/baserunning, Elo, advanced bankroll mgmt), "every pick in the open" receipts grid (152 colored W/L pills grouped by date with daily P&L column, each pill linking to its game analysis), Twitter/Discord buttons. `noindex`, not in nav. |
| **Homepage announcement card** | `site/templates/index.html`, `site/static/style.css` | Done — green-tinted card linking to `/30-days.html` above the daily picks hero. |
| **Milestone tweet + Discord embed** | `src/twitter/poster.py` (manual), `src/discord/poster.py` | Done — 30-day check-in announcement posted Apr 25 via tweepy direct call + Discord webhook (green embed with 6 stat fields). |
